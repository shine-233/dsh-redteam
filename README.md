...
```sh
dsh plugin --profile web add file:C:\path\to\dsh-redteam-<version>.tgz
```
...
| CVSS v4.0 定级 | FIRST CVSS v4.0 规范 | finding 的 `cvssVector` 接受 v3.1 或 v4.0 向量（`CVSS:4.0/AV:N/AC:L/AT:N/…`），v4 分值按官方 RedHat 计算器算法逐行移植（MacroVector + 深度再分配，270 项查找表），与 FIRST 校验向量完全一致 |
| 扫描器导入 | Metasploit workspace / Faraday ingestion | `redteam_import_scan format=nmap-xml|nessus-xml`：nmap 主机/开放端口→资产树+侦察事实，Nessus ReportItem→严重度映射的 CVE 关联漏洞（plugin_output 作复现步骤、标题+主机去重）；原始 XML 存为证据 |
| JIRA 双向桥 | DefectDojo/Jira remediaiton workflow | `redteam_jira_export` 输出 issue payload（summary/priority/labels+findingId 台账），`redteam_jira_apply` 回写 jiraKey/jiraStatus；tracker 侧 Done 不自动关闭——必须走复测留证 |
| 多用户角色门禁 | 协作 RBAC（Faraday workspace） | `redteam_add_operator` 注册 commander/operator/viewer；协作写入接受 `as` 参数过角色门（未注册即拒，viewer 只读，匿名单人不受限） |
| SLA 时限追踪 | DefectDojo SLA | 漏洞按严重度策略自动盖截止章（critical 7 / high 30 / medium 90 / low 180 天，可 slaDays 覆盖）；state 输出逾期清单与升级提示，Web 卡片显示 ⏰ 徽章 |
...
| 执行摘要与下一步建议 | PwnDoc/Dradis 报告标配 + Strix planner 风格 | markdown 报告头部自动生成执行摘要（结论/最高风险/覆盖率/复用告警）；`redteam_state` 输出 nextSteps 启发式（覆盖缺口、受阻意图、未验证凭据、仅尝试技术…） |
| 范围结构化注册表 | 授权边界数字化（ROE 条款入档） | `redteam_add_scope kind=in/out` 注册目标模式；资产/漏洞/IOC 自动判定越界（out 命中）与漏登（有 in 清单但零匹配），state/统计面板/全部报告输出合规表 |
| IOC CSV 导出 | MISP/Cuckoo 提取工作流 | `redteam_report format=ioc-csv` 输出带表头的 CSV 行（id/type/value/sample/intent/时间/上下文，含引号转义） |
| TAXII 2.1 信封导出 | oasis-open TAXII 集合响应 | `redteam_report format=taxii` 输出 `{more,next,data}` 信封包裹同一批 STIX 对象，可直接喂 OpenCTI/MISP 连接器 |
| HTML 报告 | PwnDoc 自包含页面惯例 | `redteam_report format=html` 单文件深色主题页面（内联 CSS 无 JS）：摘要芯片、严重度条形、漏洞卡片、范围合规表、时间线、证据折叠附录，打印友好 |
...
- **工具**（`src/tools.ts`）：29 个 —— `redteam_add_goal`（authorization 必填，重开 goal 关闭旧 engagement）/ `redteam_add_intent`（可选 phase）/ `redteam_add_evidence` / `redteam_add_fact`（evidenceIds 引用、可选 phase）/ `redteam_add_asset`（parentId 留空为根资产，tags 指纹）/ `redteam_add_finding`（reproducibleSteps 必填；cvssVector 支持 v3.1 与 v4.0 自动算分；techniqueIds/owaspIds 校验；SLA 按策略盖章）/ `redteam_add_credential`（凭据登记，脱敏展示）/ `redteam_update_intent`（任务树状态推进）/ `redteam_retest_finding`（复测闭环）/ `redteam_flag_finding`（DefectDojo 式分诊：误报/范围外/风险接受/审核中）/ `redteam_update_credential`(验证凭据) / `redteam_submit`（子 agent 分批直写指定父 intent）/ `redteam_add_scope`（结构化授权边界 in/out）/ `redteam_add_operator`（协作角色注册）/ `redteam_import_scan`（nmap/Nessus XML 摄取）/ `redteam_jira_export|redteam_jira_apply`（JIRA 双向桥）/ `redteam_state`（含 nextSteps 建议与凭据复用、范围合规、SLA 逾期）/ `redteam_search`（12 类记录跨表检索）/ `redteam_overview`（部署级全局度量）/ `redteam_graph` / `redteam_report`（markdown|html|json|sarif|navlayer|stix|taxii|ioc-csv）/ `redteam_engagements`（历史列表）。
