...
2. 紧接着用 redteam_add_scope 把边界登记成可判定条目：kind=in 收录允许目标（如 *.example.net、10.0.0.0/24），kind=out 收录明确禁区（如 prod-db.example.net）。此后资产/漏洞/IOC 自动判定越界与漏登，state 与报告输出合规表。
3. 把工作拆成意图：每个 redteam_add_intent 是一个可验证的探索方向（如「外网资产测绘」「VPN 入口枚举」）。用 assetIds 把方向锚定到目标资产（驱动覆盖度统计）；新方向若来自已有事实，用 derivedFrom 引用这些 fact id；多步利用链（如 注入→取凭据→横向→提权）用 dependsOn 声明前置 intent。
...
## 边界
- 只操作授权范围内的目标；scope 外的动作一律拒绝并在记录中说明。范围调整走 redteam_add_hint 留痕，同时用 redteam_add_scope 更新结构化边界。
...
2. Immediately register the boundary as decidable entries via redteam_add_scope: kind=in collects allowed targets (*.example.net, 10.0.0.0/24), kind=out collects explicit no-go zones (prod-db.example.net). Assets/findings/IOCs are then judged automatically — out-of-scope hits and unscoped targets surface in state, the stats tab, and every report.
3. Decompose work into intents: each redteam_add_intent is one verifiable exploration direction ("external asset mapping", "VPN entry enumeration"). Anchor directions to assets with assetIds (drives coverage), cite motivating facts with derivedFrom, and order multi-step exploit chains (injection → creds → lateral → privesc) with dependsOn.
...
## Boundaries
- Operate only inside scope; refuse out-of-scope actions and note the refusal in records. Scope adjustments go through redteam_add_hint for the audit trail, then update the structured boundary via redteam_add_scope.
